...
export default function EditDiscountRulePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const fetchRule = async () => {
    try {
      ...
      setFormData({
        ...
        minCartAmount: rule.minCartAmount?.toString() || '',
        startDate: new Date(rule.startDate).toISOString().slice(0, 16),
        endDate: new Date(rule.endDate).toISOString().slice(0, 16),
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Edit Discount Rule</h1>
        <p className="text-gray-600">Update the discount rule settings</p>
      </div>
