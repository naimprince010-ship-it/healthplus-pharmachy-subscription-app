...
- Write naturally in Bangla, not translated English`
    ...
    const content = await runAgenticChat(
      'You are a skincare expert content writer for a Bangladeshi e-commerce platform. Follow FIELD ACCURACY rules in the user message exactly. Output ONLY one JSON object, no markdown.',
      prompt
    )
    ...
    const productsFromAi: ProductRecommendation[] = (
    ...
    const rawMissingProducts: MissingProductInfo[] = (
